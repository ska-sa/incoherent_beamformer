...
void *spead_api_setup(struct spead_api_module_shared *s)
{
  fprintf (stderr, "SETUP");
    ...
    env_temp = getenv("THRESHOLD");
    //Set data file prefix from environment variables, or just use default of /Data1/latest
    if (env_temp != NULL) threshold = atoi(env_temp);
    ...
    env_temp = getenv("SAVE_T_P");
    //Set data file prefix from environment variables, or just use default of /Data1/latest
    if (env_temp != NULL) save_t_p = env_temp;
    else save_t_p = SAVE_T_P;
...
int spead_api_callback(struct spead_api_module_shared *s, struct spead_item_group *ig, void *data)
{
  // fprintf (stderr, "CALLBACK");
    ...
    if (ss->prior_ts == 0) {
        ...
        if (save_t_p)
            snprintf(ss->t_p_filename, 255, "%s/t_p%llu.dat", prefix, ts);
  ...
  if (w_fd2 == NULL)
    ...
    if (save_t_p)
        t_p_f = open(ss->t_p_filename, O_WRONLY | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
...
int check_pulse(int8_t * data, unsigned long long ts, unsigned long long offset)
{

  fprintf (stderr, "[%d] offset = %llu",getpid(), offset);
  int i,j;
  int totalPower;
  uint8_t im, re;
  int ret = -1;
  for (i = 0; i < ts_per_heap; i++){
    totalPower = 0;
    for (j = 2 ; j < 200; j=j+2){
      // im = data[i + j * ts_per_heap] >> 4;
      // re = (data[i + j * ts_per_heap] << 4) >> 4;
      re = (int)data[i * n_chans + j];
      im = (int)(data[i * n_chans + j+1]);
      totalPower += (re * re) + (im * im);
    }
    totalPower = totalPower / 100;
    if (save_t_p)
        pwrite (t_p_f, &totalPower, 4, 4 * (offset+i));
