...
void capture_spead(void* threadarg)
{
    ...
    fprintf(stderr, KYEL "[%d] tsync = %llu\n" RESET, tid, tsync);
    ...
    {
        try{
            //fprintf(stderr, "[%d] new spead heap\n", tid);
            pthread_mutex_lock(&tsync_mutex);
            ts = tsync;
            pthread_mutex_unlock(&tsync_mutex);
            ...
            if (master_id == tid){
                fprintf (stderr, KYEL "[%d] setting tsync\n" RESET, tid);
                pthread_mutex_lock(&tsync_mutex);
                tsync = ts2;
                pthread_mutex_unlock(&tsync_mutex);
            }
...
int send_udp (void* threadarg){
   fprintf(stderr,KRED "sending %d\n" RESET, packet_count); 
    ...
    strcpy(source_ip , "192.168.202.14");
...
void run (int port1, int port2, int port3, dada_hdu_t * hdu, int sync_time)
{
    int acc_count = 0;
    ...
    while (true)
    {
        ...
        if (master_id == 3){
            pthread_mutex_lock(&tsync_mutex);
            fprintf(stderr, KRED "[3] setting tsync\n", RESET);
            tsync = ts2;
            pthread_mutex_unlock(&tsync_mutex);
        }
	...
	uint32_t wrapped = 0;
        ...
        if (out_file == 0){
            ...
            snprintf(filename,255,"%s/%.17g.dat",FILE_LOC, seconds);
            out_file = open(filename, O_WRONLY | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
	}
        ...
        if (tsdiff2/2 * acc_len / 536870912 - read_head > num_vals)
        {
            ...
            //fprintf (stderr, "\nts = %llu, ts/12207.03125 = %llu\n",ts, seconds);
            current_time = sync_time + seconds;
...
int main (int argc, char **argv)
{
    ...
    fprintf (stderr, "synctime = %d", atoi(argv[1]));
    run (7160, 7161, 7162, hdu, atoi(argv[1]));
}
